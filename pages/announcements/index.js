...
import { add_announcement, all_announcements } from "../../api/teacher"
...
const Announcements = () => {
    ...
    const addAnnouncement = async e => {
        ...
        try {
            ...
            const anc = res.data
            res.status === 200 && showToast('success', 'Announcement added successfully'), close_modal()
        ...
        <Fragment>
            ...
            {open &&
                <Modal open={open} title='Add Announcement'>
                    <form className="grid grid-cols gap-6 py-5" onSubmit={addAnnouncement}>
                        {/* modal email */}
                        <Input
                            label='Title'
                        ...
                        />
                        <textarea value={announcement.description} className='border border-gray-200 focus:outline-none focus:border-gray-300 rounded text-gray-500 p-4 w-full' rows="2" onChange={e => setAnnouncement({ ...announcement, description: e.target.value })}></textarea>
                ...
                <div className="grid lg:grid-cols-2 gap-4">
                    {
                        announcements.length > 0
                            ? announcements.map(anc => (
                                <div className="flex justify-between items-center border rounded-lg border-gray-300 p-5 shadow-sm bg-white" key={anc.id}>
                                    <div>
                            ...
                            : <p className="text-center mt-24 font-bold text-xl text-gray-400">Add & Show Announcements here</p>
                    }
                </div>
            </div>
        </Fragment>
